import { Article, Project, Post, ArticlesData, ProjectsData, YearFilterEvent, NavToggleEvent } from './types.js';
...
const FALLBACK_POSTS: { [key: string]: Post } = {
    'ai-meal-plan': {
...
async function loadPostContent(slug: string): Promise<Post | null> {
    try {
...
function renderHomePosts(articles: Article[]): void {
...
function renderArticleList(articles: Article[], year: number): void {
...
function renderPostContent(post: Post): void {
    // Hide loading and show content
    const loadingElement = document.getElementById('loading');
    const contentElement = document.getElementById('post-content');
    
    if (loadingElement) loadingElement.style.display = 'none';
    if (contentElement) contentElement.classList.add('loaded');
    
    const titleElement = document.getElementById('post-title');
    const dateElement = document.getElementById('post-date');
    const tagElement = document.getElementById('post-tag');
    const imageElement = document.getElementById('post-image') as HTMLImageElement;
    const bodyElement = document.getElementById('post-body');
    
    if (titleElement) titleElement.textContent = post.title;
    if (dateElement) dateElement.textContent = new Date(post.date).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' });
    if (tagElement) tagElement.innerHTML = renderTag(post.tag);
...
// Event handlers
function handleYearFilterClick(event: Event): void {
    const btn = (event.target as HTMLElement).closest('button[data-year]') as HTMLButtonElement | null;
    if (!btn) return;
    ...
    if (location.pathname.endsWith('post.html')) {
        ...
        loadPostContent(slug).then(post => {
            ...
            if (post) {
                ...
                const loadingElement = document.getElementById('loading');
                if (loadingElement) loadingElement.innerHTML = 'Article not found';
            }
        }).catch(error => {
            console.error('Error loading post:', error);
            const loadingElement = document.getElementById('loading');
            if (loadingElement) loadingElement.innerHTML = 'Error loading article';
        });
    }
});
...
window.addEventListener('load', () => {
    ...
    setTimeout(() => {
        ...
        (window as any).scrollAnimator = animator;
    }, 100);
});